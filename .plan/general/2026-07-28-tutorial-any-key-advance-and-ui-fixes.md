...
- **회의록과 이슈 본문이 어긋난다.** 회의록(2번째 스프린트 회의록)은 "게임 튜토리얼 수정 - **enter를
  누르는 걸로**"라고 적혀 있고 이슈 목록에도 "튜토리얼 enter 키 수정"으로 걸려 있다. 반면 ARTEL-155
  본문은 "**any 키**로 튜토리얼 진행하도록 변경, any로 넘겨야한다는 것을 플레이어에게 알림"이다.
  이슈 본문이 회의록보다 나중에 작성됐고 더 구체적이라 본문(any 키)을 따랐다. any 키는 Enter를
  포함하므로 회의록 요구사항도 함께 만족한다. 문구 확정이 필요하면 알려달라.
- 회의록에 **Back 버튼 위치에 대한 언급은 없다.** "잘리지 않게 화면 안으로"를 요구사항으로 보고
  우측 하단 + 여백 40px로 배치했다.
